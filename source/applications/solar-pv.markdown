...
An emonPi can be used on it's own if both the generation and consumption feeds are in the same physical location and WiFi/Ethernet connectivity is accesible at this location. The emonPi is a Raspberry Pi based energy monitor, Emoncms web-app runs on the emonPi allowing data to be logged locally to the  emonPi's SD card as well as (optionally) logging to [Emoncms.org](http://emoncms.org) cloud server.
...
The Emoncms setup instructions below are applicable to both the emonPi and the emonTx where CT1 is solar generation and CT2 is either consumption (Type 1 system) or import/export (Type 2 system). If using an emonTx to monitor generation or consumption substitute the 'log to feed' instructions to use an input from the emonTx instead of emonPi.
...
**Type 1 solar PV System:** When the generation and consumption **can** be monitored separately. The amount exported/imported to or from the grid is the difference between generation and consumption.

<p class='note'>
Type 1 system:  Grid (import/export) = Consumption – Generation
</p>

**Type 2 solar PV System:** When the generation and consumption **cannot** be monitored separately e.g. the PV inverter output is fed into the fuse box and ther household loads are connected to other circuits in the same fuse box. If this is the case, the output from the PV inverter and the grid import/export connection will need to be monitored and site consumption calculated by subtracting.
...
<p class='note'>
The polarity of the power readings depends on the orientation of the clip-on CT sensor. Orient the CTs so that generation and consumption is positive and grid import/export is **positive when importing and negative when exporting**. The correct orientation can be determined by trial and error.
</p>
